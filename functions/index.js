...
exports.createEmployee = functions.https.onCall(async (data, context) => {
  try {
    ...
    if (isAdmin && !userIsOmniAdmin) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Only Asa can create admin users"
      );
...
exports.removeEmployeeCompany = functions.https.onCall(
  async (data, context) => {
    try {
      if (!context.auth) {
        throw new functions.https.HttpsError(
      ...
      const requesterDataDocRef = admin
        .firestore()
        .collection(context.auth.uid)
        .doc(ADMIN_DOC_NAME);
      ...
      throw new functions.https.HttpsError(
        "internal",
        "Error removing user information",
        error
      );
    }
  }
...
exports.transferEmployeeData = functions.https.onCall(async (data, context) => {
  try {
    ...
    throw new functions.https.HttpsError(
      "internal",
      "Error transferring user information",
      error
    );
  }
});
