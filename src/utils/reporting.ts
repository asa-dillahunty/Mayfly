...
import { getCompanyEmployeeList } from "./firebaseQueries";
import {
  ABBREVIATIONS,
  buildDocName,
  getEndOfWeekString,
...
export const createPrintable = (dataObject, selectedDate: Date) => {
  ...
  getCompanyEmployeeList(dataObject.id, selectedDate, docName).then(
    async (empList) => {
      ...
      for (let i = 0; i < empList.length; i++) {
        ...
        newDoc.text(
          `$${(
            Math.round(empList[i].rate * empList[i].hoursPaidThisWeek * 100) /
            100
          ).toFixed(2)}`,
          70,
          line * lineHeight,
